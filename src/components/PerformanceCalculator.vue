              ...
              <div class="relative mt-1">
                ...
                <div class="absolute inset-y-0 right-0 flex items-center">
                  <div class="h-full pr-3 flex items-center border-l border-gray-300 w-[100px] justify-end">
                    <div class="flex flex-col -space-y-px">
                      <button 
                        @click="devamsizlik = Math.min(4, Number(devamsizlik) + 1)"
                      ...
                      <button 
                        @click="devamsizlik = Math.max(0, Number(devamsizlik) - 1)"
                        class="px-1.5 py-1 text-gray-500 hover:text-red-600 focus:outline-none"
                      >
                        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
...
const calculateDevamsizlikPuani = (gunSayisi: number) => {
  ...
  const devamsizlikBilgisi = devamsizlikTablosu.find(d => d.gun === gunSayisi)
  return devamsizlikBilgisi ? devamsizlikBilgisi.puan : -13 // 4 günden fazlası için maksimum kesinti
}
